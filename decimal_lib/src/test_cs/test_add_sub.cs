...
public class Example
{
  ...
  public static void print_s21_is_equal(decimal number1, decimal number2, int num)
  {
    ...
    Console.WriteLine("#test s21_s21_is_sub{0}\n", num);
    Console.WriteLine("  s21_decimal src1, src2, res, our_res;\n");
    ...
    int[] bits3 = decimal.GetBits(result);
    Console.WriteLine("  res.bits[0] = 0b{0};\n  res.bits[1] = 0b{1};\n  res.bits[2] = 0b{2};\n  res.bits[3] = 0b{3};\n",
          Convert.ToString(bits3[0], 2).PadLeft(32, '0'),
          Convert.ToString(bits3[1], 2).PadLeft(32, '0'),
          Convert.ToString(bits3[2], 2).PadLeft(32, '0'),
          Convert.ToString(bits3[3], 2).PadLeft(32, '0'));

    Console.WriteLine("  s21_is_sub(src1, src2, our_res);\n");
    Console.WriteLine("  int result = s21_is_equal(res, our_res);\n");
  // Console.WriteLine("  int origin = {0};\n", origin.ToString());
    Console.WriteLine("  ck_assert_int_eq(1, result);\n");
  }
}
